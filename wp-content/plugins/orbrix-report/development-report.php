...
<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default" style="background: white;    margin: 15px;    padding: 15px;    margin-top: 40px;border: solid;">
					<div class="panel-heading" style="border-bottom: solid 2px;    margin-bottom: 15px;">
						<h1>Development Request Report</h1>
					</div>
					<!-- /.panel-heading -->
					<div class="panel-body">
						<div class="popup-gallery">
							<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-admin">
											...
											<?php
												global $wpdb;
												$results = $wpdb->get_results("SELECT * FROM `wpfr_contact_developer` order by created_date DESC");
		...
		function submitForm(row_id){
			...
			jQuery.ajax({
				...
				data:'action=contect_developer_followup&row_id='+row_id+'&followup='+followup,
				beforeSend: function () {
		...
		for (var i = 0; i < elems.length; i++) {
			...
			elems[i].onchange = function(e) {
				...
				//ajax call 
				var data = {
					action: 'change_development_status',
					id: data_id,
		...
		function delete_slider(id){
			var r = confirm("Are you sure want to remove Development Request Report?");
			if (r == true) {
				
				var data = {
					action: 'delete_development_request',
					id: id
				};
				jQuery.post('<?php echo plugins_url( 'ajax_plugin.php', __FILE__ );?>', data, function(response) {
					if(response=="success"){
